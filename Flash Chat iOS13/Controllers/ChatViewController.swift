...
class ChatViewController: UIViewController {
    ...
    func loadMessages () {
        ...
        db.collection(K.FStore.collectionName)
            ...
            self.messages = []
            if let e = error {
                print("There was an issues with retrieving data from Firestore. \(e)")
            } else {
    ...
    @IBAction func sendPressed(_ sender: UIButton) {
        if let messageBody = messageTextfield.text, let messageSender = Auth.auth().currentUser?.email {
            db.collection(K.FStore.collectionName).addDocument(data: [K.FStore.senderField : messageSender, K.FStore.bodyField : messageBody,K.FStore.dateField : Date().timeIntervalSince1970]) { (error) in
                if let e = error {
                    print("There was an issue with saving data in firestore - \(e)")
                } else {
                    print("Succsessfully saved data.")
                    DispatchQueue.main.async {
    ...
    @IBAction func logOutPressed(_ sender: UIBarButtonItem) {
        ...
        do {
            ...
            //method that get us back to the root view controller!
